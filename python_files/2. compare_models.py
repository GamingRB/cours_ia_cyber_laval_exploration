...
def compute_net_cost(y_true, y_pred):
    tn, fp, fn, tp = confusion_matrix(
        y_true, y_pred, labels=["other", "North Central"]
    ).ravel()
    cost = 10 * fp + 1 * fn
    gain = 5 * tp + 2 * tn
    return gain - cost
...
for name, model in [
    ...
    net_cost = compute_net_cost(y_test, y_pred)
    results[name] = net_cost
...
def get_train_test_gap(model, X, y, cv=5):
...
for name, model in [
    ...
    train_acc, test_acc, gap = get_train_test_gap(model, X, y, cv=5)
    gaps[name] = {
        "train_acc": train_acc,
        "test_acc": test_acc,
        "gap": gap,
